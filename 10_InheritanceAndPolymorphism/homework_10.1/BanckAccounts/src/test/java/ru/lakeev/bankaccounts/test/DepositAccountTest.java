...
@DisplayName("М6 — тест класса ru.lakeev.bank_accounts.accounts.DepositAccount")
public class DepositAccountTest {
    ...
    void putNegativeAmount() {
        depositAccount.put(-1.0);
        assertEquals(0.0, depositAccount.getAccAmount(), DELTA, notExpectedSumMessage);
    }
    ...
    void takeTooMuchMoney() {
        ...
        assertEquals(2.0, depositAccount.getAccAmount(), DELTA, notExpectedSumMessage);
    }
    ...
    private void rollBackTime(int year, int months, int days) {
        try {
            ...
            Assert.fail("Поле lastIncome не найдено в классе ru.lakeev.bank_accounts.accounts.DepositAccount.");
    ...
    @Test
    @DisplayName("Метод send, попытка отправить деньги через месяц после пополнения")
    void sendInMonth() {
        depositAccount.put(2.0);
        rollBackTime(0, 1, 1);
        DepositAccount accountReceiver = (DepositAccount)
