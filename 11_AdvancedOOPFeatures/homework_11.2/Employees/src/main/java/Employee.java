...
public class Employee {
    ...
    try {
      ...
      for (String line : lines) {
        ...
        staff.add(new Employee(
            ...
            LocalDate.parse(fragments[2], DateTimeFormatter
                .ofPattern(dateFormat).localizedBy(Locale.ENGLISH))
        ));
      }
    } catch (Exception ex) {
      ex.printStackTrace();
  ...
  public String toString() {
    return name + " - " + salary + " - " +
        (workStart.format(DateTimeFormatter
            .ofPattern("dd.MM.yyyy").localizedBy(Locale.ENGLISH)));
  }
